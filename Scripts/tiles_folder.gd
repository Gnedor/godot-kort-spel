...
func _process(delta: float) -> void:
	if dragged_tile:
		...
		sort_by_x_position(owned_tiles)
...
func align_tiles():
	...
	for tile in owned_tiles:
		if tile != dragged_tile and !tile.is_placed:
			var tile_pos_x = (window_size.x / 2) - (FOLDER_BOUNDRY / 2) + (FOLDER_BOUNDRY * (i + 1) / (owned_tiles.size() + 1))
...
func animate_card_snap(tile, position, speed):
	...
	if tile.global_position.y != self.position.y:
		tween.tween_property(tile, "global_position", position, find_duration(tile.position, position, speed * 2)).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	else:
