...
func adjust_mod_details(mod_name : String, amount : int, mod_trait : String):
	...
	var value = ModifierDatabase.MODIFIERS[mod_name]["start"] + (ModifierDatabase.MODIFIERS[mod_name]["stack"] * (amount - 1))
	var new_word = "[color=#41ffa3]" + str(value) + "[/color]"
	var new_string = ModifierDatabase.MODIFIERS[mod_name]["description"].replace("PLACEHOLDER", new_word)
	description.description_label.text = new_string
